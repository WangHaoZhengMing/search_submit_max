...
/// 上传 PDF 到腾讯云 COS（内部函数，使用已有凭证）
async fn upload_pdf_to_cos_with_credential(
...
pub async fn upload_pdf(local_file_path: &str) -> Result<String> {
    ...
    // 2. 上传 PDF 到腾讯云 COS
    let pdf_url = upload_pdf_to_cos_with_credential(&credential, local_file_path).await?;

    info!("PDF 上传完成，URL: {}", pdf_url);
    Ok(pdf_url)
}
...
mod tests {
    ...
    async fn test_upload_pdf() {
        ...
        if std::path::Path::new(file_path).exists() {
            ...
            println!("未找到测试文件 {}，仅测试凭证获取", file_path);
            // 回退到仅测试凭证
